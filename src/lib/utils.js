...
module.exports = {
    usageMessage: function (channel) {
        const embed = new Discord.MessageEmbed()
            ...
            .addField(
                "Quoting",
                "!quote author [name] => quote a given person with entries in the DB. (author can be replaced with 'random')\n\n!quote add [name] [topic] [quote] => add a quote attributed to a given person.\n\n!author add [name] [picture_url] [full_name] => add a new author of quotes\n\n!author list => names all authors that exist in the DB.",
                true
            )
            .addField(
                "Pictures",
                "!pic show [name] => retrieve the picture with the given name. (show can be replaced with 'random')\n\n!pic add [name] [url] => add a new picture to the DB to be called later with !pic\n\n!pic list => lists all pictures that exist in the DB.",
                true
            )
            .addField(
                "Ascii Art",
                "!ascii [phrase] => print out the given phrase in ascii art.",
                true
            )
            .addField(
                "Emoji Multiplier",
                "[emoji] * [number] => print out the given emoji [number] times.",
                true
            )
            .addField(
                "Updates",
                "!update author [pic|name] [resource name] [url|full name]\n\n!update pic [name|url] [resource name] [name|url]",
                true
            )
            .addField(
                "Birthdays",
    ...
    */
    ...
    getMsgInfo: function (msg, regex = null, client = null) {
        let msgArr = msg.content.split(/(\s+)/).filter(function (e) {
