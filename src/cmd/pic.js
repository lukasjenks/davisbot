...
class Pic {
    ...
    picAdd(channel) {
        db.run(
            "insert into picture (command, url) values (?, ?)",
            [this.name, this.url],
            (err) => {
                if (err) {
                    channel.send(
                        "An error occured. Usage: !pic add [name] [picture url]. Error: " +
                            err
                    );
                } else {
    ...
    picShow(channel) {
        if (this.subCmd === "random") {
            db.get(
                "select url from picture order by random() limit 1",
                (err, picRecord) => {
                    if (err) {
                        msg.channel.send(
                            "An error occured. Usage: !pic show [picture name]. Error: " +
                                err
                        );
            ...
            db.get(
                "select url from picture where command = ?",
                [this.name],
                (err, picRecord) => {
                    if (err) {
                        channel.send(
    ...
    picLike(channel) {
        db.get(
            "select url from picture where command like ? order by random() limit 1",
            "%" + this.name + "%",
            (err, picRecord) => {
                if (err) {
                    channel.send(
                        'An error occured. Usage: !pic like "quote substring here". Error: ' +
                            err
                    );
                } else if (picRecord !== undefined) {
                    channel.send(picRecord.url);
                    ...
                    channel.send(
                        "No pic which has a name containing the substring specified was found in the DB."
                    );
                }
            }
        );
    ...
    picList(channel) {
        db.all(
            `select command from picture order by command`,
            [],
            (err, picRecs) => {
                if (err) {
