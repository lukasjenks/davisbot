...
class Update {
    ...
    updateAuthorName(channel) {
        db.run(
            "update author set full_name = ? where command = ?",
            [this.newName, this.resourceName],
            (err) => {
                if (err) {
                    channel.send("An error occured. Error: " + err);
                } else {
                    channel.send(
                        "Successfully changed the author's full name."
                    );
                }
            }
        );
    ...
    updateAuthorPic(channel) {
        db.run(
            "update author set picture_url = ? where command = ?",
    ...
    updatePicUrl(channel) {
        db.run(
            "update picture set url = ? where command = ?",
            [this.url, this.resourceName],
            (err) => {
                if (err) {
                    channel.send("An error occured. Error: " + err);
                    ...
                    channel.send(
                        "Successfully changed the specified picture's url."
                    );
                }
            }
        );
    ...
    updatePicName(channel) {
        db.run(
            "update picture set command = ? where url = ?",
            [this.newName, this.url],
            (err) => {
                if (err) {
                    channel.send("An error occured. Error: " + err);
                } else {
                    channel.send(
                        "Successfully changed pic name associated with specified url."
                    );
                }
            }
        );
            ...
            switch (msgInfo.msgArr[2]) {
                case "name":
                    fields = msgInfo.content.match(
                        msgInfo.regex.updateAuthorNameCmd
                    );
                    break;
                case "pic":
                    fields = msgInfo.content.match(
                        msgInfo.regex.updateAuthorPicCmd
                    );
                    break;
                default:
                    break;
